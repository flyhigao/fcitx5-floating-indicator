...
X11/fcitx5 版（`../src/fcitx5-floating-indicator.py`）的 macOS 移植。一个小型「中/英」浮动窗口，**鼠标移动时显示、停止移动 10 秒后自动隐藏**，输入时自动隐藏，可盖过全屏应用。
...
- **鼠标移动即显示，停止移动 10 秒后自动隐藏**（无鼠标移动不常驻屏幕）。
...
设计目标是“不做事时几乎不耗电”。实测（macOS 11，豆包输入法激活时）：

| 指标 | 数值 |
|---|---|
| 常驻 CPU | **~1.4%**（单核） |
| 内存 | ~16 MB |
| 截图 + OCR | 仅在切换瞬间执行一次（50~200ms 后台线程），平时零开销 |

各监控任务的频率与优化手段：

| 任务 | 频率 | 说明 |
|---|---|---|
| 豆包窗口扫描 | 5 Hz | 小窗存活 2 秒以上，检测余量充足；仅豆包激活时扫描，且用 `optionOnScreenOnly` 只枚举可见窗口 |
| Shift 键轮询 | 100 Hz | HID 状态查询，捕捉极快击键 |
| 鼠标跟随 | 4 Hz | 与 X11 版一致；位置未变时跳过窗口移动；兼做鼠标移动检测（移动即显示，停 10 秒隐藏） |
| 输入源轮询 | ~3 Hz | TIS 属性查询，开销可忽略 |
| 截图 + OCR | 事件触发 | 只截 26×26 像素的小窗，Vision 后台线程执行 |

优化历程：50Hz 全量扫描 + 60Hz 跟随（~13% CPU）→ 按需扫描 + 可见窗口过滤（~4%）→ 跟随降频（~3%）→ 扫描/轮询/跟随全面降频（~1.4%）。
